...
GE, WS, lobby는 StatefulSet보다 Deployment가 현재 구조에 더 맞다.

이유:

```text
- GE/WS/lobby는 PVC나 디스크 상태를 복원하지 않는다.
...
- lobby도 안정 ordinal identity나 headless DNS가 필요하지 않다.
```
...
Lobby StatefulSet -> Deployment
KOPIC_NODE_ID -> Pod UID를 포함한 재기동별 고유 id
HPA target -> GE/WS Deployment
GE/WS Deployment.spec.replicas -> manifest에서 제거
Lobby replicas -> 1 유지
```
...
lobby도 같은 prefix를 사용해야 한다. dev lobby manifest에도 같은 계열의 값을 추가한다.
...
```text
apps/kopic/dev/lobby/deployment.yaml
apps/kopic/dev/lobby/statefulset.yaml
apps/kopic/dev/lobby/kustomization.yaml
```

`deployment.yaml`은 새로 만들고, 기존 `statefulset.yaml`은 삭제하거나 kustomization에서 제외한다.

lobby는 이번 autoscaling 대상이 아니므로 HPA를 추가하지 않는다. 기존 `replicas: 1`은 Deployment에 유지한다.

```yaml
apiVersion: apps/v1
kind: Deployment
metadata:
  name: kopic-dev-lobby
spec:
  replicas: 1
  selector:
    matchLabels:
...
lobby 변경의 목적은 GE/WS와 마찬가지로 불필요한 StatefulSet identity를 제거하고, dev Kopic 워크로드 형태를 Deployment 중심으로 맞추는 것이다.
...
```text
GE/WS Deployment manifest에서 spec.replicas를 작성하지 않는다.
...
- lobby Deployment replicas: 1

HPA가 관리하는 것:
- GE/WS Deployment의 실제 replica 수
```
...
lobby는 기존 ClusterIP Service와 Ingress를 유지한다.

Deployment 전환 후 headless Service는 필수는 아니지만, 제거는 1차 목적과 직접 관련이 없다.

```text
1차 권장: 기존 kopic-dev-lobby Service 유지, headless Service는 당장 제거하지 않음
후속 정리: StatefulSet 제거 안정화 후 headless Service 삭제 검토
```
...
1. GE/WS/lobby `deployment.yaml` 작성
2. GE/WS/lobby `kustomization.yaml`에서 `statefulset.yaml`을 `deployment.yaml`로 교체
...
7. GE/WS Deployment manifest에 `spec.replicas`가 없는지 확인
8. lobby Deployment manifest에 `replicas: 1`이 유지되는지 확인
9. kustomize build 또는 ArgoCD dry-run 성격의 검증 수행
10. dev ArgoCD sync
11. 필요 시에만 `apps/kopic/application-dev.yaml`에 ArgoCD replica ignore 추가
12. 다음 시나리오를 확인
...
```text
1. GE/WS/lobby를 StatefulSet에서 Deployment로 전환
