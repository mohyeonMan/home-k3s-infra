...
단일 PC k3s 노드 기준으로 CPU보다 memory request가 먼저 제약이 된다.
현재 관측 기준으로 GE/WS Pod의 실사용 메모리는 대체로 다음 범위에 있다.
...
따라서 GE/WS의 기본 request는 실제 idle 사용량보다 약간 여유를 둔 `512Mi`를 기준으로 한다.
limit은 순간 증가 여지를 남기되 단일 PC에서 과도한 메모리 overcommit을 피하기 위해 `2Gi`를 기준으로 한다.
...
CPU request는 기존 `300m`을 유지한다.
현재 노드에서는 CPU 사용률이 낮고, HPA의 CPU target은 `300m` request 기준 평균 `70%`를 유지한다.

HPA memory target은 `AverageValue: 1Gi`를 기준으로 한다.
기존 `2Gi` target은 `3Gi` limit과 함께 쓰기에는 scale-up 신호가 너무 늦다.
`1Gi`는 평상시 사용량과 충분히 떨어져 있으면서, Pod당 메모리 사용량이 의미 있게 커졌을 때 scale-up이 발생하는 기준이다.
...
dev는 평소 각 1개 Pod가 기본이고, 실제 필요 시에만 2개까지 오른다.
prod도 평소 각 1개 Pod를 기본으로 보고, 부하가 있을 때 3개까지 오른다.

단일 PC 기준 최대 Pod 수는 다음과 같다.

```text
평상시: dev 2개 + prod 2개 = GE/WS 총 4개
최대치: dev 4개 + prod 6개 = GE/WS 총 10개
```

`512Mi` request 기준이면 GE/WS 전체 memory request는 평상시 약 `2Gi`, 최대 약 `5Gi`다.
`1Gi` request를 유지하면 최대 약 `10Gi`를 GE/WS만 예약하게 되므로, 현재 PC에서는 prod까지 확장할 때 스케줄링 여유가 부족해질 수 있다.

우선 dev 매니페스트에 이 기준을 먼저 적용한다.
prod는 나중에 StatefulSet/Deployment 전환과 함께 같은 리소스 기준을 적용하되, 운영 관측에서 GE/WS가 꾸준히 `700Mi` 이상을 사용하면 prod만 `640Mi` request로 올리는 방안을 검토한다.
